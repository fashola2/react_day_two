...
class Factory {
  ...
  customerBuild(color, options){
    this.color = color
    this.options = options
    console.log("Building one " + this.color + " " + this.options.trim + " " + this.options.model + " with the following options: " + this.options.options);
  }
...
class Car extends Factory{
  constructor(warranty, model, doors, color, enginetype, transmission, trim, wheelstrim, audio, seatstrim, moonroof){
    super(warranty)
    this.model = model
    this.doors = doors
    this.color = color
...
let zoot = new Factory("Mazda","USA", true, true, "60,000 miles/3 years")
...
let mazda3 = new Car('Mazda3',4,"Black","hybrid","automatic","Touring","Base","Premium","leather",true)
...
miataRf.massBuild(15000, {color: "Red", trim: "Grand Touring", model: "Miata-Rf"})
...
miataRf.massBuild(35000, {color: "blue", trim: "sport", model: "Trail Blazer"})
